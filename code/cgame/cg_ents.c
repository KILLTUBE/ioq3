...
void CG_PositionEntityOnTag( refEntity_t *entity, const refEntity_t *parent, 
	...
	// lerp the tag
	trap_cgame_R_LerpTag( &lerped, parentModel, parent->oldframe, parent->frame,
...
void CG_SetEntitySoundPosition( centity_t *cent ) {
	if ( cent->currentState.solid == SOLID_BMODEL ) {
		...
		trap_cgame_S_UpdateEntityPosition( cent->currentState.number, origin );
	} else {
		trap_cgame_S_UpdateEntityPosition( cent->currentState.number, cent->lerpOrigin );
	}
}
...
static void CG_EntityEffects( centity_t *cent ) {
	...
	if ( cent->currentState.loopSound ) {
		if (cent->currentState.eType != ET_SPEAKER) {
			trap_cgame_S_AddLoopingSound( cent->currentState.number, cent->lerpOrigin, vec3_origin, 
				cgs.gameSounds[ cent->currentState.loopSound ] );
		} else {
			trap_cgame_S_AddRealLoopingSound( cent->currentState.number, cent->lerpOrigin, vec3_origin, 
				cgs.gameSounds[ cent->currentState.loopSound ] );
		}
	}
	...
	if(cent->currentState.constantLight)
	{
		...
		trap_cgame_R_AddLightToScene(cent->lerpOrigin, i, r, g, b);
	}
...
static void CG_Speaker( centity_t *cent ) {
	...
	trap_cgame_S_StartSound (NULL, cent->currentState.number, CHAN_ITEM, cgs.gameSounds[cent->currentState.eventParm] );
...
static void CG_Item( centity_t *cent ) {
	...
	if ( item->giType == IT_WEAPON ) {
		...
		ent.nonNormalizedAxes = qtrue;
#ifdef MISSIONPACK
		trap_cgame_S_AddLoopingSound( cent->currentState.number, cent->lerpOrigin, vec3_origin, cgs.media.weaponHoverSound );
#endif
	}
	...
	if ( item->giType == IT_WEAPON && wi && wi->barrelModel ) {
		...
		trap_cgame_R_AddRefEntityToScene( &barrel );
	}
...
/*
	...
	if ( weapon->missileDlight ) {
		trap_cgame_R_AddLightToScene(cent->lerpOrigin, weapon->missileDlight, 
...
*/
	...
	if ( weapon->missileSound ) {
		...
		trap_cgame_S_AddLoopingSound( cent->currentState.number, cent->lerpOrigin, velocity, weapon->missileSound );
	}
...
static void CG_Grapple( centity_t *cent ) {
	...
	// add missile sound
	if ( weapon->missileSound ) {
		trap_cgame_S_AddLoopingSound( cent->currentState.number, cent->lerpOrigin, vec3_origin, weapon->missileSound );
	}
#endif
...
static void CG_TeamBase( centity_t *cent ) {
	...
	if ( cgs.gametype == GT_CTF) {
		...
		trap_cgame_R_AddRefEntityToScene( &model );
	}
#ifdef MISSIONPACK
		...
		if ( cent->currentState.frame == 2) {
			...
			if (t > h) {
				if ( !cent->muzzleFlashTime ) {
					trap_cgame_S_StartSound (cent->lerpOrigin, ENTITYNUM_NONE, CHAN_BODY,  cgs.media.obeliskRespawnSound);

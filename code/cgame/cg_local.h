...
// abort the game
void		trap_cgame_Error(const char *fmt) __attribute__((noreturn));
...
int			trap_cgame_FS_Seek( fileHandle_t f, long offset, int origin ); // fsOrigin_t
...
clipHandle_t trap_cgame_CM_InlineModel( int index );		// 0 = world, 1+ = bmodels
...
void		trap_cgame_S_StopLoopingSound(int entnum);
...
sfxHandle_t	trap_cgame_S_RegisterSound( const char *sample, qboolean compressed );		// returns buzz if not found
void		trap_cgame_S_StartBackgroundTrack( const char *intro, const char *loop );	// empty name stops music
...
qhandle_t	trap_cgame_R_RegisterModel( const char *name );			// returns rgb axis if not found
qhandle_t	trap_cgame_R_RegisterSkin( const char *name );			// returns all white if not found
qhandle_t	trap_cgame_R_RegisterShader( const char *name );			// returns all white if not found
qhandle_t	trap_cgame_R_RegisterShaderNoMip( const char *name );			// returns all white if not found
...
void		trap_cgame_R_AddPolysToScene( qhandle_t hShader , int numVerts, const polyVert_t *verts, int numPolys );
...
void		trap_cgame_R_SetColor( const float *rgba );	// NULL = 1,1,1,1

...
void BotSetUserInfo(bot_state_t *bs, char *key, char *value) {
	...
	trap_game_GetUserinfo(bs->client, userinfo, sizeof(userinfo));
	Info_SetValueForKey(userinfo, key, value);
	trap_game_SetUserinfo(bs->client, userinfo);
	...
	if ( bs->lastgoal_ltgtype ) {
		...
		if ( gametype == GT_CTF ) {
			if ( bs->ltgtype == LTG_GETFLAG ) {
				...
				tb = BotTeamFlag(bs);
				eb = BotEnemyFlag(bs);
				tt = trap_game_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, tb->areanum, TFL_DEFAULT);
				et = trap_game_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, eb->areanum, TFL_DEFAULT);
...
void BotRefuseOrder(bot_state_t *bs) {
	...
	if ( bs->order_time && bs->order_time > FloatTime() - 10 ) {
		trap_game_EA_Action(bs->client, ACTION_NEGATIVE);
...
int BotPointAreaNum(vec3_t origin) {
	...
	areanum = trap_game_AAS_PointAreaNum(origin);
	if (areanum) return areanum;
	VectorCopy(origin, end);
	end[2] += 10;
	numareas = trap_game_AAS_TraceAreas(origin, end, areas, NULL, 10);
...
char *ClientName(int client, char *name, int size) {
	...
	trap_game_GetConfigstring(CS_PLAYERS+client, buf, sizeof(buf));
...
int ClientFromName(char *name) {
	...
	for (i = 0; i < level.maxclients; i++) {
		trap_game_GetConfigstring(CS_PLAYERS+i, buf, sizeof(buf));
		Q_CleanStr( buf );
...
void BotChooseWeapon(bot_state_t *bs) {
	...
	if (bs->cur_ps.weaponstate == WEAPON_RAISING ||
		...
		trap_game_EA_SelectWeapon(bs->client, bs->weaponnum);
	}
	else {
		newweaponnum = trap_game_BotChooseBestFightWeapon(bs->ws, bs->inventory);
...
void BotSetupForMovement(bot_state_t *bs) {
	...
	//
	trap_game_BotInitMoveState(bs->ms, &initmove);
}
		...
		if (offence) {
			if (!(bs->teamtaskpreference & TEAMTP_ATTACKER)) {
				...
				if (BotTeamLeader(bs)) {
					...
					//trap_game_BotEnterChat(bs->cs, leader, CHAT_TELL);
				}
...
qboolean BotIsObserver(bot_state_t *bs) {
	...
	trap_game_GetConfigstring(CS_PLAYERS+bs->client, buf, sizeof(buf));
...
qboolean BotInLavaOrSlime(bot_state_t *bs) {
	...
	feet[2] -= 23;
	return (trap_game_AAS_PointContents(feet) & (CONTENTS_LAVA|CONTENTS_SLIME));
}
...
int BotCanAndWantsToRocketJump(bot_state_t *bs) {
	...
	rocketjumper = trap_game_Characteristic_BFloat(bs->character, CHARACTERISTIC_WEAPONJUMPING, 0, 1);
...
void BotGoCamp(bot_state_t *bs, bot_goal_t *goal) {
	...
	//get the team goal time
	camper = trap_game_Characteristic_BFloat(bs->character, CHARACTERISTIC_CAMPER, 0, 1);
...
int BotWantsToCamp(bot_state_t *bs) {
	...
	for (cs = trap_game_BotGetNextCampSpotGoal(0, &goal); cs; cs = trap_game_BotGetNextCampSpotGoal(cs, &goal)) {
		traveltime = trap_game_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, goal.areanum, TFL_DEFAULT);
...
void BotDontAvoid(bot_state_t *bs, char *itemname) {
	...
	num = trap_game_BotGetLevelItemGoal(-1, itemname, &goal);
	while(num >= 0) {
		trap_game_BotRemoveFromAvoidGoals(bs->gs, goal.number);
		num = trap_game_BotGetLevelItemGoal(num, itemname, &goal);
	}
}
...
void BotRoamGoal(bot_state_t *bs, vec3_t goal) {
	...
	for (i = 0; i < 10; i++) {
		...
		if (len > 200) {
			...
			if (!trace.startsolid) {
				trace.endpos[2]++;
				pc = trap_game_PointContents(trace.endpos, bs->entitynum);
...
bot_moveresult_t BotAttackMove(bot_state_t *bs, int tfl) {
	...
	if (bs->attackchase_time > FloatTime()) {
		...
		trap_game_BotMoveToGoal(&moveresult, bs->ms, &goal, tfl);
		return moveresult;
	}
	//
	memset(&moveresult, 0, sizeof(bot_moveresult_t));
	//
	attack_skill = trap_game_Characteristic_BFloat(bs->character, CHARACTERISTIC_ATTACK_SKILL, 0, 1);
	jumper = trap_game_Characteristic_BFloat(bs->character, CHARACTERISTIC_JUMPER, 0, 1);
	croucher = trap_game_Characteristic_BFloat(bs->character, CHARACTERISTIC_CROUCHER, 0, 1);
	...
	if (attack_skill <= 0.4) {
		...
		if (dist > attack_dist + attack_range) {
			if (trap_game_BotMoveInDirection(bs->ms, forward, 400, movetype)) return moveresult;
		}
		if (dist < attack_dist - attack_range) {
			if (trap_game_BotMoveInDirection(bs->ms, backward, 400, movetype)) return moveresult;
		}
		return moveresult;
	}
	...
	for (i = 0; i < 2; i++) {
		...
		if (trap_game_BotMoveInDirection(bs->ms, sideward, 400, movetype))
			return moveresult;
...
float BotEntityVisible(int viewer, vec3_t eye, vec3_t viewangles, float fov, int ent) {
	...
	//
	pc = trap_game_AAS_PointContents(eye);
	...
	for (i = 0; i < 3; i++) {
		...
		if (trap_game_AAS_PointContents(middle) & (CONTENTS_LAVA|CONTENTS_SLIME|CONTENTS_WATER)) {
		...
		if (trace.fraction >= 1 || trace.ent == hitent) {
			...
			otherinfog = (trap_game_AAS_PointContents(middle) & CONTENTS_FOG);
			if (infog && otherinfog) {
...
int BotFindEnemy(bot_state_t *bs, int curenemy) {
	...
	alertness = trap_game_Characteristic_BFloat(bs->character, CHARACTERISTIC_ALERTNESS, 0, 1);
	easyfragger = trap_game_Characteristic_BFloat(bs->character, CHARACTERISTIC_EASY_FRAGGER, 0, 1);
...
void BotAimAtEnemy(bot_state_t *bs) {
	...
	//
	aim_skill = trap_game_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_SKILL, 0, 1);
	aim_accuracy = trap_game_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY, 0, 1);
	//
	if (aim_skill > 0.95) {
		//don't aim too early
		reactiontime = 0.5 * trap_game_Characteristic_BFloat(bs->character, CHARACTERISTIC_REACTIONTIME, 0, 1);
		if (bs->enemysight_time > FloatTime() - reactiontime) return;
		if (bs->teleport_time > FloatTime() - reactiontime) return;
	}

	//get the weapon information
	trap_game_BotGetWeaponInfo(bs->ws, bs->weaponnum, &wi);
	//get the weapon specific aim accuracy and or aim skill
	if (wi.number == WP_MACHINEGUN) {
		aim_accuracy = trap_game_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_MACHINEGUN, 0, 1);
	}
	else if (wi.number == WP_SHOTGUN) {
		aim_accuracy = trap_game_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_SHOTGUN, 0, 1);
	}
	else if (wi.number == WP_GRENADE_LAUNCHER) {
		aim_accuracy = trap_game_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_GRENADELAUNCHER, 0, 1);
		aim_skill = trap_game_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_SKILL_GRENADELAUNCHER, 0, 1);
	}
	else if (wi.number == WP_ROCKET_LAUNCHER) {
		aim_accuracy = trap_game_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_ROCKETLAUNCHER, 0, 1);
		aim_skill = trap_game_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_SKILL_ROCKETLAUNCHER, 0, 1);
	}
	else if (wi.number == WP_LIGHTNING) {
		aim_accuracy = trap_game_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_LIGHTNING, 0, 1);
	}
	else if (wi.number == WP_RAILGUN) {
		aim_accuracy = trap_game_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_RAILGUN, 0, 1);
	}
	else if (wi.number == WP_PLASMAGUN) {
		aim_accuracy = trap_game_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_PLASMAGUN, 0, 1);
		aim_skill = trap_game_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_SKILL_PLASMAGUN, 0, 1);
	}
	else if (wi.number == WP_BFG) {
		aim_accuracy = trap_game_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_BFG10K, 0, 1);
		aim_skill = trap_game_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_SKILL_BFG10K, 0, 1);
	}
	//
	...
	if (enemyvisible) {
		...
		if (wi.speed) {
			...
			if (!(dist > 100 && VectorLengthSquared(dir) < Square(32))) {
				...
				if (aim_skill > 0.8 &&
					...
					trap_game_AAS_PredictClientMovement(&move, bs->enemy, origin,
		...
		if (aim_skill > 0.5) {
			...
			if (wi.number == WP_BFG ||
				...
				VectorSet(goal.maxs, 8, 8, 8);
				//
				if (trap_game_BotPredictVisiblePosition(bs->lastenemyorigin, bs->lastenemyareanum, &goal, TFL_DEFAULT, target)) {
					VectorSubtract(target, bs->eye, dir);
	...
	if (bot_challenge.integer) {
		...
		if (aim_accuracy > 0.9 && bs->enemysight_time < FloatTime() - 1) {
			...
			trap_game_EA_View(bs->client, bs->viewangles);
		}
	}
}
...
void BotCheckAttack(bot_state_t *bs) {
	...
	//
	reactiontime = trap_game_Characteristic_BFloat(bs->character, CHARACTERISTIC_REACTIONTIME, 0, 1);
	if (bs->enemysight_time > FloatTime() - reactiontime) return;
	if (bs->teleport_time > FloatTime() - reactiontime) return;
	//if changing weapons
	...
	firethrottle = trap_game_Characteristic_BFloat(bs->character, CHARACTERISTIC_FIRETHROTTLE, 0, 1);
...
void BotMapScripts(bot_state_t *bs) {
	...
	trap_game_GetServerinfo(info, sizeof(info));
...
int BotFuncButtonActivateGoal(bot_state_t *bs, int bspent, bot_activategoal_t *activategoal) {
	...
	trap_game_AAS_ValueForBSPEpairKey(bspent, "model", model, sizeof(model));
	if (!*model)
		return qfalse;
	...
	trap_game_AAS_FloatForBSPEpairKey(bspent, "lip", &lip);
	if (!lip) lip = 4;
	//get the move direction from the angle
	trap_game_AAS_FloatForBSPEpairKey(bspent, "angle", &angle);
	...
	dist *= 0.5;
	//
	trap_game_AAS_FloatForBSPEpairKey(bspent, "health", &health);
	...
	if (health) {
		...
		if (bsptrace.fraction >= 1.0 || bsptrace.ent == entitynum) {
			...
			trap_game_AAS_PresenceTypeBoundingBox(PRESENCE_CROUCH, bboxmins, bboxmaxs);
			for (i = 0; i < 3; i++) {
			...
			numareas = trap_game_AAS_TraceAreas(start, end, areas, points, 10);
			//
			for (i = numareas-1; i >= 0; i--) {
				if (trap_game_AAS_AreaReachability(areas[i])) {
					break;
				}
			}
			if (i < 0) {
		...
		numareas = trap_game_AAS_TraceAreas(start, end, areas, NULL, 10);
		//
...
void BotEnableActivateGoalAreas(bot_activategoal_t *activategoal, int enable) {
	...
	for (i = 0; i < activategoal->numareas; i++)
		trap_game_AAS_EnableRoutingArea( activategoal->areas[i], enable );
...
int BotGetActivateGoal(bot_state_t *bs, int entitynum, bot_activategoal_t *activategoal) {
	...
	for (ent = trap_game_AAS_NextBSPEntity(0); ent; ent = trap_game_AAS_NextBSPEntity(ent)) {
		if (!trap_game_AAS_ValueForBSPEpairKey(ent, "model", tmpmodel, sizeof(tmpmodel))) continue;
	...
	trap_game_AAS_ValueForBSPEpairKey(ent, "classname", classname, sizeof(classname));
	if (!*classname) {
	...
	if (!strcmp(classname, "func_door")) {
		if (trap_game_AAS_FloatForBSPEpairKey(ent, "health", &health)) {
		...
		//
		trap_game_AAS_IntForBSPEpairKey(ent, "spawnflags", &spawnflags);
		// if the door starts open then just wait for the door to return
		if ( spawnflags & 1 )
			return 0;
		//get the door origin
		if (!trap_game_AAS_VectorForBSPEpairKey(ent, "origin", origin)) {
			VectorClear(origin);
		}
		...
		trap_game_AAS_ValueForBSPEpairKey(ent, "model", model, sizeof(model));
		if (*model) {
			modelindex = atoi(model+1);
			if (modelindex) {
				BotModelMinsMaxs(modelindex, ET_MOVER, 0, absmins, absmaxs);
				//
				numareas = trap_game_AAS_BBoxAreas(absmins, absmaxs, areas, MAX_ACTIVATEAREAS*2);
				...
				for (i = 0; i < numareas; i++) {
					...
					trap_game_AAS_AreaInfo(areas[i], &areainfo);
	...
	if (!trap_game_AAS_ValueForBSPEpairKey(ent, "targetname", targetname[0], sizeof(targetname[0]))) {
	...
	cur_entities[0] = trap_game_AAS_NextBSPEntity(0);
	for (i = 0; i >= 0 && i < 10;) {
		for (ent = cur_entities[i]; ent; ent = trap_game_AAS_NextBSPEntity(ent)) {
			if (!trap_game_AAS_ValueForBSPEpairKey(ent, "target", target, sizeof(target))) continue;
			if (!strcmp(targetname[i], target)) {
				cur_entities[i] = trap_game_AAS_NextBSPEntity(ent);
				break;
			}
		}
		if (!ent) {
		...
		if (!trap_game_AAS_ValueForBSPEpairKey(ent, "classname", classname, sizeof(classname))) {
			if (bot_developer.integer) {
		...
		if (!strcmp(classname, "func_button")) {
			...
			if ( trap_game_AAS_AreaReachability(bs->areanum) ) {
				// disable all areas the blocking entity is in
				BotEnableActivateGoalAreas( activategoal, qfalse );
				//
				t = trap_game_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, activategoal->goal.areanum, bs->tfl);
		...
		// the actual button or trigger might be linked through a target_relay or target_delay
			...
			if (trap_game_AAS_ValueForBSPEpairKey(ent, "targetname", targetname[i+1], sizeof(targetname[0]))) {
				i++;
				cur_entities[i] = trap_game_AAS_NextBSPEntity(0);
			}
		}
	}
#ifdef OBSTACLEDEBUG
...
void BotPrintActivateGoalInfo(bot_state_t *bs, bot_activategoal_t *activategoal, int bspent) {
	...
	trap_game_AAS_ValueForBSPEpairKey(bspent, "classname", classname, sizeof(classname));
	...
	trap_game_EA_Say(bs->client, buf);
}
...
void BotRandomMove(bot_state_t *bs, bot_moveresult_t *moveresult) {
	...
	trap_game_BotMoveInDirection(bs->ms, dir, 400, MOVE_WALK);
...
void BotAIBlocked(bot_state_t *bs, bot_moveresult_t *moveresult, int activate) {
	...
	if (activate && entinfo.modelindex > 0 && entinfo.modelindex <= max_bspmodelindex) {
		...
		if (bspent) {
			...
			if (!(moveresult->flags & MOVERESULT_ONTOPOFOBSTACLE) &&
				trap_game_AAS_AreaReachability(bs->areanum))
				return;
		}
		else {
			// enable any routing areas that were disabled
	...
	if (movetype != MOVE_CROUCH || !trap_game_BotMoveInDirection(bs->ms, hordir, 400, movetype)) {
		// perform the movement
		if (!trap_game_BotMoveInDirection(bs->ms, sideward, 400, movetype)) {
			...
			trap_game_BotMoveInDirection(bs->ms, sideward, 400, movetype);
		}
	}
	//
...
int BotAIPredictObstacles(bot_state_t *bs, bot_goal_t *goal) {
	...
	trap_game_AAS_PredictRoute(&route, bs->areanum, bs->origin,
...
void BotCheckConsoleMessages(bot_state_t *bs) {
	...
	//
	while((handle = trap_game_BotNextConsoleMessage(bs->cs, &m)) != 0) {
		//if the chat state is flooded with messages the bot will read them quickly
		if (trap_game_BotNumConsoleMessages(bs->cs) < 10) {
		...
		if (!BotMatchMessage(bs, m.message)) {
			...
			if (m.type == CMS_CHAT && !bot_nochat.integer) {
				//
				if (!trap_game_BotFindMatch(m.message, &match, MTCONTEXT_REPLYCHAT)) {
					trap_game_BotRemoveConsoleMessage(bs->cs, handle);
					continue;
				}
				...
				//
				trap_game_BotMatchVariable(&match, NETNAME, netname, sizeof(netname));
				trap_game_BotMatchVariable(&match, MESSAGE, message, sizeof(message));
				...
				//unify the message
				trap_game_UnifyWhiteSpaces(message);
				//
				trap_game_Cvar_Update(&bot_testrchat);
				if (bot_testrchat.integer) {
					//
					trap_game_BotLibVarSet("bot_testrchat", "1");
					//if bot replies with a chat message
					if (trap_game_BotReplyChat(bs->cs, message, context, CONTEXT_REPLY,
															NULL, NULL,
															NULL, NULL,
															NULL, NULL,
															botname, netname)) {
				...
				//if at a valid chat position and not chatting already and not in teamplay
					...
					chat_reply = trap_game_Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_REPLY, 0, 1);
...
void BotCheckForGrenades(bot_state_t *bs, entityState_t *state) {
	...
	trap_game_BotAddAvoidSpot(bs->ms, state->pos.trBase, 160, AVOID_ALWAYS);
}
...
void BotCheckEvents(bot_state_t *bs, entityState_t *state) {
	...
	switch(event) {
		...
		case EV_GLOBAL_SOUND:
		{
			...
			trap_game_GetConfigstring(CS_SOUNDS + state->eventParm, buf, sizeof(buf));
			/*
...
void BotCheckSnapshot(bot_state_t *bs) {
	...
	trap_game_BotAddAvoidSpot(bs->ms, vec3_origin, 0, AVOID_CLEAR);
	//reset kamikaze body
	bs->kamikazebody = 0;
...
void BotCheckAir(bot_state_t *bs) {
	if (bs->inventory[INVENTORY_ENVIRONMENTSUIT] <= 0) {
		if (trap_game_AAS_PointContents(bs->eye) & (CONTENTS_WATER|CONTENTS_SLIME|CONTENTS_LAVA)) {
			return;
		}
	}
...
bot_goal_t *BotAlternateRoute(bot_state_t *bs, bot_goal_t *goal) {
	...
	if (bs->altroutegoal.areanum) {
		...
		t = trap_game_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, bs->altroutegoal.areanum, bs->tfl);
		if (t && t < 20) {
...
#ifdef MISSIONPACK
	if (gametype == GT_CTF) {
		if (trap_game_BotGetLevelItemGoal(-1, "Neutral Flag", &ctf_neutralflag) < 0)
			BotAI_Print(PRT_WARNING, "No alt routes without Neutral Flag\n");
		if (ctf_neutralflag.areanum) {
			//
			red_numaltroutegoals = trap_game_AAS_AlternativeRouteGoals(
			...
			blue_numaltroutegoals = trap_game_AAS_AlternativeRouteGoals(
										ctf_neutralflag.origin, ctf_neutralflag.areanum,
		...
		if (trap_game_BotGetLevelItemGoal(-1, "Neutral Obelisk", &neutralobelisk) < 0)
...
void BotDeathmatchAI(bot_state_t *bs, float thinktime) {
	...
	if (bs->setupcount > 0) {
		...
		trap_game_Characteristic_String(bs->character, CHARACTERISTIC_GENDER, gender, sizeof(gender));
		//set the bot gender
		trap_game_GetUserinfo(bs->client, userinfo, sizeof(userinfo));
		Info_SetValueForKey(userinfo, "sex", gender);
		trap_game_SetUserinfo(bs->client, userinfo);
		//set the chat gender
		if (gender[0] == 'm') trap_game_BotSetChatGender(bs->cs, CHAT_GENDERMALE);
		else if (gender[0] == 'f')  trap_game_BotSetChatGender(bs->cs, CHAT_GENDERFEMALE);
		else  trap_game_BotSetChatGender(bs->cs, CHAT_GENDERLESS);
		//set the chat name
		ClientName(bs->client, name, sizeof(name));
		trap_game_BotSetChatName(bs->cs, name, bs->client);
		//
	...
	if (i >= MAX_NODESWITCHES) {
		trap_game_BotDumpGoalStack(bs->gs);
		trap_game_BotDumpAvoidGoals(bs->gs);
...
int BotGoalForBSPEntity( char *classname, bot_goal_t *goal ) {
	...
	for (ent = trap_game_AAS_NextBSPEntity(0); ent; ent = trap_game_AAS_NextBSPEntity(ent)) {
		if (!trap_game_AAS_ValueForBSPEpairKey(ent, "classname", value, sizeof(value)))
			continue;
...
void BotSetupDeathmatchAI(void) {
	...
	gametype = trap_game_Cvar_VariableIntegerValue("g_gametype");

	trap_game_Cvar_Register(&bot_rocketjump, "bot_rocketjump", "1", 0);
	trap_game_Cvar_Register(&bot_grapple, "bot_grapple", "0", 0);
	trap_game_Cvar_Register(&bot_fastchat, "bot_fastchat", "0", 0);
	trap_game_Cvar_Register(&bot_nochat, "bot_nochat", "0", 0);
	trap_game_Cvar_Register(&bot_testrchat, "bot_testrchat", "0", 0);
	trap_game_Cvar_Register(&bot_challenge, "bot_challenge", "0", 0);
	trap_game_Cvar_Register(&bot_predictobstacles, "bot_predictobstacles", "1", 0);
	trap_game_Cvar_Register(&g_spSkill, "g_spSkill", "2", 0);
	//
	if (gametype == GT_CTF) {
		if (trap_game_BotGetLevelItemGoal(-1, "Red Flag", &ctf_redflag) < 0)
			BotAI_Print(PRT_WARNING, "CTF without Red Flag\n");
		if (trap_game_BotGetLevelItemGoal(-1, "Blue Flag", &ctf_blueflag) < 0)
		...
		if (trap_game_BotGetLevelItemGoal(-1, "Red Obelisk", &redobelisk) < 0)
			BotAI_Print(PRT_WARNING, "Overload without Red Obelisk\n");
		BotSetEntityNumForGoalWithActivator(&redobelisk, "team_redobelisk");
		if (trap_game_BotGetLevelItemGoal(-1, "Blue Obelisk", &blueobelisk) < 0)
	...
	max_bspmodelindex = 0;
	for (ent = trap_game_AAS_NextBSPEntity(0); ent; ent = trap_game_AAS_NextBSPEntity(ent)) {
		if (!trap_game_AAS_ValueForBSPEpairKey(ent, "model", model, sizeof(model))) continue;
		if (model[0] == '*') {

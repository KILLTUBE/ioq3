...
int BotClientTravelTimeToGoal(int client, bot_goal_t *goal) {
	...
	if (!areanum) return 1;
	return trap_game_AAS_AreaTravelTimeToGoalArea(areanum, ps.origin, goal->areanum, TFL_DEFAULT);
}
...
void BotSayTeamOrderAlways(bot_state_t *bs, int toclient) {
	...
	if (bs->client == toclient) {
		...
		trap_game_BotGetChatMessage(bs->cs, buf, sizeof(buf));
		ClientName(bs->client, name, sizeof(name));
		Com_sprintf(teamchat, sizeof(teamchat), EC"(%s"EC")"EC": %s", name, buf);
		trap_game_BotQueueConsoleMessage(bs->cs, CMS_CHAT, teamchat);
	}
	else {
		trap_game_BotEnterChat(bs->cs, toclient, CHAT_TELL);
	}
}
...
void BotVoiceChat(bot_state_t *bs, int toclient, char *voicechat) {
	...
	if (toclient == -1)
		// voice only say team
		trap_game_EA_Command(bs->client, va("vsay_team %s", voicechat));
	else
		// voice only tell single player
		trap_game_EA_Command(bs->client, va("vtell %d %s", toclient, voicechat));
#endif
}
...
void BotVoiceChatOnly(bot_state_t *bs, int toclient, char *voicechat) {
	...
	if (toclient == -1)
		// voice only say team
		trap_game_EA_Command(bs->client, va("vosay_team %s", voicechat));
	else
		// voice only tell single player
		trap_game_EA_Command(bs->client, va("votell %d %s", toclient, voicechat));
#endif
}

...
float trap_game_Cvar_VariableValue( const char *var_name ) {
	...
	trap_game_Cvar_VariableStringBuffer(var_name, buf, sizeof(buf));
	return atof(buf);
}
...
static void G_LoadArenasFromFile( char *filename ) {
	...
	len = trap_game_FS_FOpenFile( filename, &f, FS_READ );
	if ( !f ) {
		trap_game_Print( va( S_COLOR_RED "file not found: %s\n", filename ) );
		return;
	}
	if ( len >= MAX_ARENAS_TEXT ) {
		trap_game_FS_FCloseFile( f );
		trap_game_Print( va( S_COLOR_RED "file too large: %s is %i, max allowed is %i\n", filename, len, MAX_ARENAS_TEXT ) );
		return;
	}

	trap_game_FS_Read( buf, len, f );
	buf[len] = 0;
	trap_game_FS_FCloseFile( f );
...
static void G_LoadArenas( void ) {
	...
	trap_game_Cvar_Register( &arenasFile, "g_arenasFile", "", CVAR_INIT|CVAR_ROM );
	...
	numdirs = trap_game_FS_GetFileList("scripts", ".arena", dirlist, 1024 );
	dirptr  = dirlist;
	...
	trap_game_Print( va( "%i arenas parsed\n", g_numArenas ) );
...
void G_AddRandomBot( int team ) {
	...
	skill = trap_game_Cvar_VariableValue( "g_spSkill" );
	if (team == TEAM_RED) teamstr = "red";
	else if (team == TEAM_BLUE) teamstr = "blue";
	else teamstr = "free";
	trap_game_SendConsoleCommand( EXEC_INSERT, va("addbot random %f %s %i\n", skill, teamstr, 0) );
}
...
int G_RemoveRandomBot( int team ) {
	...
	for ( i=0 ; i< g_maxclients.integer ; i++ ) {
		...
		trap_game_SendConsoleCommand( EXEC_INSERT, va("clientkick %d\n", i) );
		return qtrue;
	}
	return qfalse;
}
...
void G_CheckMinimumPlayers( void ) {
	...
	trap_game_Cvar_Update(&bot_minplayers);
...
void G_CheckBotSpawn( void ) {
	...
	for( n = 0; n < BOT_SPAWN_QUEUE_DEPTH; n++ ) {
		...
		if( g_gametype.integer == GT_SINGLE_PLAYER ) {
			trap_game_GetUserinfo( botSpawnQueue[n].clientNum, userinfo, sizeof(userinfo) );
...
qboolean G_BotConnect( int clientNum, qboolean restart ) {
	...
	if (!BotAISetupClient( clientNum, &settings, restart )) {
		trap_game_DropClient( clientNum, "BotAISetupClient failed" );
		return qfalse;
	}
...
static void G_AddBot( const char *name, float skill, const char *team, int delay, char *altname) {
	...
	clientNum = trap_game_BotAllocateClient();
	if ( clientNum == -1 ) {
	...
	if ( Q_stricmp( name, "random" ) == 0 ) {
		...
		if ( botinfoNum < 0 ) {
			...
			trap_game_BotFreeClient( clientNum );
			return;
		}
	...
	if (!*s ) {
		trap_game_Print( S_COLOR_RED "Error: bot has no aifile specified\n" );
		trap_game_BotFreeClient( clientNum );
		return;
	}
	...
	trap_game_SetUserinfo( clientNum, userinfo );
...
void Svcmd_AddBot_f( void ) {
	...
	// name
	trap_game_Argv( 1, name, sizeof( name ) );
	if ( !name[0] ) {
		trap_game_Print( "Usage: Addbot <botname> [skill 1-5] [team] [msec delay] [altname]\n" );
		return;
	}

	// skill
	trap_game_Argv( 2, string, sizeof( string ) );
	if ( !string[0] ) {
		skill = 4;
	}
	else {
	...
	// team
	trap_game_Argv( 3, team, sizeof( team ) );

	// delay
	trap_game_Argv( 4, string, sizeof( string ) );
	if ( !string[0] ) {
		delay = 0;
	}
	else {
	...
	// alternative name
	trap_game_Argv( 5, altname, sizeof( altname ) );
	...
	if ( level.time - level.startTime > 1000 &&
		trap_game_Cvar_VariableIntegerValue( "cl_running" ) ) {
		trap_game_SendServerCommand( -1, "loaddefered\n" );	// FIXME: spelled wrong, but not changing for demo
	}
}
...
void Svcmd_BotList_f( void ) {
	...
	trap_game_Print("^1name             model            aifile              funname\n");
	for (i = 0; i < g_numBots; i++) {
		...
		trap_game_Print(va("%-16s %-16s %-20s %-20s\n", name, model, aifile, funname));
	}
}
...
static void G_SpawnBots( char *botList, int baseDelay ) {
	...
	skill = trap_game_Cvar_VariableValue( "g_spSkill" );
	if( skill < 1 ) {
		trap_game_Cvar_Set( "g_spSkill", "1" );
		skill = 1;
	}
	else if ( skill > 5 ) {
		trap_game_Cvar_Set( "g_spSkill", "5" );
		skill = 5;
	}
	...
	while( *p ) {
		...
		// does "Bad Things"
		trap_game_SendConsoleCommand( EXEC_INSERT, va("addbot %s %f free %i\n", bot, skill, delay) );
...
static void G_LoadBots( void ) {
	...
	trap_game_Cvar_Register( &botsFile, "g_botsFile", "", CVAR_INIT|CVAR_ROM );
	if( *botsFile.string ) {
	...
	numdirs = trap_game_FS_GetFileList("scripts", ".bot", dirlist, 1024 );
	dirptr  = dirlist;
	for (i = 0; i < numdirs; i++, dirptr += dirlen+1) {
		dirlen = strlen(dirptr);
		strcpy(filename, "scripts/");
	...
	trap_game_Print( va( "%i bots parsed\n", g_numBots ) );
}
...
char *G_GetBotInfoByNumber( int num ) {
	if( num < 0 || num >= g_numBots ) {
		trap_game_Print( va( S_COLOR_RED "Invalid bot number: %i\n", num ) );
		return NULL;
	}
...
void G_InitBots( qboolean restart ) {
	...
	trap_game_Cvar_Register( &bot_minplayers, "bot_minplayers", "0", CVAR_SERVERINFO );
	...
	if( g_gametype.integer == GT_SINGLE_PLAYER ) {
		...
		if ( fragLimit ) {
			trap_game_Cvar_Set( "fraglimit", strValue );
		}
		else {
		...
		if ( timeLimit ) {
			trap_game_Cvar_Set( "timelimit", strValue );
		}
		else {
		...
		if ( !fragLimit && !timeLimit ) {
			trap_game_Cvar_Set( "fraglimit", "10" );
			trap_game_Cvar_Set( "timelimit", "0" );
		}

...
gentity_t	*G_TestEntityPosition( gentity_t *ent ) {
	...
	if ( ent->client ) {
		trap_game_Trace( &tr, ent->client->ps.origin, ent->r.mins, ent->r.maxs, ent->client->ps.origin, ent->s.number, mask );
	} else {
		trap_game_Trace( &tr, ent->s.pos.trBase, ent->r.mins, ent->r.maxs, ent->s.pos.trBase, ent->s.number, mask );
	}
...
qboolean G_CheckProxMinePosition( gentity_t *check ) {
	...
	trap_game_Trace( &tr, start, NULL, NULL, end, check->s.number, MASK_SOLID );
...
qboolean G_MoverPush( gentity_t *pusher, vec3_t move, vec3_t amove, gentity_t **obstacle ) {
	...
	*obstacle = NULL;
	...
	trap_game_UnlinkEntity( pusher );

	listedEntities = trap_game_EntitiesInBox( totalMins, totalMaxs, entityList, MAX_GENTITIES );

	// move the pusher to its final position
	VectorAdd( pusher->r.currentOrigin, move, pusher->r.currentOrigin );
	VectorAdd( pusher->r.currentAngles, amove, pusher->r.currentAngles );
	trap_game_LinkEntity( pusher );
...
void Reached_BinaryMover( gentity_t *ent ) {
	...
	if ( ent->moverState == MOVER_1TO2 ) {
		...
		if ( ent->teammaster == ent || !ent->teammaster ) {
			trap_game_AdjustAreaPortalState( ent, qfalse );
		}
	} else {
...
void Use_BinaryMover( gentity_t *ent, gentity_t *other, gentity_t *activator ) {
	...
	if ( ent->moverState == MOVER_POS1 ) {
		...
		// open areaportal
		if ( ent->teammaster == ent || !ent->teammaster ) {
			trap_game_AdjustAreaPortalState( ent, qtrue );
		}
		return;
	}
...
void SP_func_door (gentity_t *ent) {
	...
	trap_game_SetBrushModel( ent, ent->model );
...
void SP_func_train (gentity_t *self) {
	...
	trap_game_SetBrushModel( self, self->model );
	InitMover( self );
